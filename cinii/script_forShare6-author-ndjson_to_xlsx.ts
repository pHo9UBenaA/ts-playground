// docker compose exec node ./cinii/node_modules/.bin/ts-node ./cinii/script_forShare6-author-ndjson_to_xlsx.ts
...
const saveAuthorCareerInstitutionInstitutionIdentifierTable = async (
	...
	data.forEach((item) => {
		Object.keys(item).map((key) => {
			if (!authorCareerInstitutionInstitutionIdentifierColumn.some((column) => column.key === key)) {
				errorColumns.push(key);
			}
		});
	});
	if (errorColumns.length) {
		throw new Error(`authorCareerInstitutionInstitutionIdentifierColumnに存在しないキーがあります: ${errorColumns}`);
	}
...
const saveAuthorCareerInstitutionNotationTable = async (
	...
	if (errorColumns.length) {
		throw new Error(`authorCareerInstitutionNotationColumnに存在しないキーがあります: ${errorColumns}`);
	}
...
const saveAuthorCareerDepartmentDepartmentIdentifierTable = async (
	...
	data.forEach((item) => {
		Object.keys(item).map((key) => {
			if (!authorCareerDepartmentDepartmentIdentifierColumn.some((column) => column.key === key)) {
				errorColumns.push(key);
			}
		});
	});
	if (errorColumns.length) {
		throw new Error(`authorCareerDepartmentDepartmentIdentifierColumnに存在しないキーがあります: ${errorColumns}`);
	}
...
const saveAuthorCareerDepartmentNotationTable = async (
	...
	if (errorColumns.length) {
		throw new Error(`authorCareerDepartmentNotationColumnに存在しないキーがあります: ${errorColumns}`);
	}
...
const saveAuthorCareerJobTitleJobTitleIdentifierTable = async (
	...
	data.forEach((item) => {
		Object.keys(item).map((key) => {
			if (!authorCareerJobTitleJobTitleIdentifierColumn.some((column) => column.key === key)) {
				errorColumns.push(key);
			}
		});
	});
	if (errorColumns.length) {
		throw new Error(`authorCareerJobTitleJobTitleIdentifierColumnに存在しないキーがあります: ${errorColumns}`);
	}
...
const saveAuthorCareerJobTitleNotationTable = async (
	...
	if (errorColumns.length) {
		throw new Error(`authorCareerJobTitleNotationColumnに存在しないキーがあります: ${errorColumns}`);
	}
...
const saveAuthorCareerTable = async (
	...
	const authorCareerColumn = (() => {
		const result = authorColumns.filter((column) => {
			return column.key === '@id' || column.key === 'career-custom-id' || column.key === 'career-since' || column.key === 'career-until';
		});
		return result;
	})();
	...
	const getAuthorCareerRows = (data: CiNiiResearchResponse[]) => {
		...
		data.forEach((item) => {
			...
			item.career?.forEach((career, index) => {
				const row = {
					'@id': item['@id'],
					'career-custom-id': `${item['@id']}-${index}`,
				...
				if (career.institution) {
					...
					institutionInstitutionIdentifierData = [
						...institutionInstitutionIdentifierData,
						{ '@id': item['@id'], 'career-custom-id': `${item['@id']}-${index}` },
					];
					institutionNotationData = [
						...institutionNotationData,
						{ '@id': item['@id'], 'career-custom-id': `${item['@id']}-${index}` },
			...
			if (!item.career) {
				...
				rows.push({ '@id': item['@id'], 'career-custom-id': item['@id'] });
			}
			formattedData = [...formattedData, ...rows];
		});
		return {
	...
	await saveAuthorCareerInstitutionNotationTable(workbook, filePath, relationData.institutionNotationData);
	await saveAuthorCareerDepartmentDepartmentIdentifierTable(
		workbook,
		filePath,
		relationData.departmentDepartmentIdentifierData
	);
	await saveAuthorCareerDepartmentNotationTable(workbook, filePath, relationData.departmentNotationData);
	await saveAuthorCareerJobTitleJobTitleIdentifierTable(
		workbook,
		filePath,
		relationData.jobTitleJobTitleIdentifierData
	);
	await saveAuthorCareerJobTitleNotationTable(workbook, filePath, relationData.jobTitleNotationData);
...
const saveAllDataSameSheetXls = async (workbook: Workbook, data: CiNiiResearchResponse[]) => {
	const authorMainColumns  = checkAuthorColumns;
...
const convertAuthorNDJSONToExcel = async (year: number) => {
	const filePath = `${dirPrefix}/_tmp_jsonl_to_xlsx/${year}_author_oneTable.xlsx`;
