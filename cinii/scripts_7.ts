...
const isMedicalJournal = (article: CiNiiResearchResponse): boolean => {
	...
	const includedProductIdentifier = article.productIdentifier?.some((identifier) => {
		return identifier['@value']?.startsWith('https://search.jamas.or.jp/link/ui');
	});
...
const readIdDataAndFetchCiNiiAndSave = async (year: number) => {
	const appendArticlesFilePath = `${dirPrefix}/7-2_aggregate_results/${year}_articles.jsonl`;
	const appendPickArticlesFilePath = `${dirPrefix}/7-2_aggregate_results/2024_2023_2022_aggregate_results.xlsx`;
	const appendPickArticlesSplitYearFilePath = `${dirPrefix}/7-2_aggregate_results/${year}_aggregate_results.xlsx`;
	const appendIgnoreNoAuthorPickArticlesFilePath = `${dirPrefix}/7-2_aggregate_results/2024_2023_2022_aggregate_results_ignore_no_author.xlsx`;
	const appendIgnoreNoAuthorPickArticlesSplitYearFilePath = `${dirPrefix}/7-2_aggregate_results/${year}_aggregate_results_ignore_no_author.xlsx`;
	const appendIgnoreNoAuthorAndNoAffiliationPickArticlesFilePath = `${dirPrefix}/7-2_aggregate_results/2024_2023_2022_aggregate_results_ignore_no_author_and_no_affiliation.xlsx`;
	const appendIgnoreNoAuthorAndNoAffiliationPickArticlesSplitYearFilePath = `${dirPrefix}/7-2_aggregate_results/${year}_aggregate_results_ignore_no_author_and_no_affiliation.xlsx`;
	...
	for await (const articleKV of pipeline) {
		...
		if (!isMedicalJournal(articlesValue)) {
			continue;
		}

		articles.push(articlesValue);
		pickArticles.push({
			'@id': articlesValue['@id'],
...
(async () => {
	for await (const year of [2022, 2023, 2024]) {
